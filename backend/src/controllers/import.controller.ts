  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      for (const row of data) {
        ...
        if (!Mercado) continue; // Mercado es obligatorio para la agrupación
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      for (let i = 0; i < data.length; i++) {
        ...
        const whereExisting: any = { projectId, itemName: String(Producto) };
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      for (const row of data) {
        ...
        if (!Premio || Monto_Minimo === undefined || Monto_Maximo === undefined) continue;
